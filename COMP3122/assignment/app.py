...
print("Key:", github_key)
print("Endpoint:", github_endpoint)
print("Model Name:", github_model)
...
def generate_fun_summary(session_transcript, selected_languages): 
    ...
    system_prompt = "You are a helpful assistant."
    more_fun_prompt = f"Please generate a summary only in more fun as lightheart emoji with the given section of the transcript: {session_transcript} and please change the content with the selected language {selected_languages}."    
...
if "num_section" not in st.session_state:
    # st.session_state["num_section"] = 3
    st.session_state["num_section"] = random.randint(4, 6)  # Generate once and store 
